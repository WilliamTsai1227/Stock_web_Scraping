...
if __name__ =="__main__":                        #當在本地執行，沒有被其他程式import時
    Scraping()
    st = pd.DataFrame(data_list)
    st.to_excel("stock_data.xlsx",index=False,engine="openpyxl") #會將爬蟲結果存入名叫stock_data.xlsx 的Excel檔案
    # app = Flask(__name__)        #使用Flask套件
    # @app.route("/")              #route URL為首頁時，執行home並渲染result=data_list進templates資料夾裡面的index.html
    # def home():
    #     return render_template("index.html",result = data_list)
    # app.run(debug=True)
